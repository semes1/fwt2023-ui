...
void TouchGFXHAL::initialize()
{
    ...
    TouchGFXGeneratedHAL::initialize();
	  ...
	  W25QXX_Init();//w25qxx外部存储器初始化
	//设置资源缓冲区的地址和大小,最后一个参数是用来设置动态图片的数量,默认为0,可以不用传
		Bitmap::setCache((uint16_t*)(Bank5_SDRAM_ADDR+framesize*3),BITMAP_CACHE_SIZE);
	  Bitmap::cacheAll();
//	  setTouchSampleRate(1);  //20ms
...
bool TouchGFXHAL::blockCopy(void* RESTRICT dest, const void* RESTRICT src, uint32_t numBytes)
{
  ...
  {
		W25QXX_Read((uint8_t*)dest,(uint32_t)src-EX_FLASH_START_ADDR,numBytes);
    return true;
  }else
	{
